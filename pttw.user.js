...
(function(){
    ...
    function scriptsUI(){
        ...
        twScripts.forEach(e=>{
            ...
            btnDel.addEventListener('click', ev=>{
                ...
                localStorage.twScripts = JSON.stringify(twScripts);
                abox.close();
                scriptsUI();
            });
    ...
    const miniMap = async function(){
        ...
        mm.style.merge({position: 'absolute', left: 0, top: 0, border: 'solid 3px white'});
    ...
    function chatSearchUI(){
        let abox = box({
            header: 'Поиск в чате',
            text: `
                <input type="text" size="40" id="pttw-search-inp" autocomplete="off"><br>
                <div>
                    <div style="padding-right: 3em; display: inline;"><input type="checkbox" id="pttw-search-checkbox-msgtext" checked> Текст</div>
                    <div style="display: inline;"><input type="checkbox" id="pttw-search-checkbox-name"> Имя</div>
                </div>
                <details>
                    <summary>Настройки</summary>
                    <div style="display: inline;"><input type="checkbox" id="pttw-search-checkbox-regex"> Использовать регулярные выражения</div>
                </details><br>
                <div style="display: flex; justify-content: space-evenly;">
                    <button id="pttw-search-ok-btn" class="btn btn-success">Найти</button>
                    <button id="pttw-search-reset-btn" class="btn btn-danger">Сбросить</button>
                </div>
            `
        });
        let text = qs('#pttw-search-inp'),
            cbt = qs('#pttw-search-checkbox-msgtext'),
            cbn = qs('#pttw-search-checkbox-name'),
            cbr = qs('#pttw-search-checkbox-regex'),
            btnok = qs('#pttw-search-ok-btn'),
            btnr = qs('#pttw-search-reset-btn');
    ...
    function spamBotUI(){
        let abox = box({
            ...
            text: `
                <table><tr>
                    <td>Сообщение</td>
                    <td style="padding-left: 3em;"><input type="text" id="pttw-spam-bot-msg" maxlength="68"></td>
                </tr><tr>
                    <td>Кол-во повторов</td>
                    <td style="padding-left: 3em;"><input type="number" id="pttw-spam-bot-repeats" value="10"></td>
                </tr><tr>
                    <td>Задержка (в мс)</td>
                    <td style="padding-left: 3em;"><input type="number" id="pttw-spam-bot-delay" value="400"></td>
                </tr></table><br>
                <div style="display: flex; justify-content: center;">
                    <button id="pttw-spam-bot-ok-btn" class="btn btn-success">Начать</button>
                </div>
            `
        });
        qs('#pttw-spam-bot-ok-btn').addEventListener('click', ()=>{
            let txt = qs('#pttw-spam-bot-msg').value+'', rep = +qs('#pttw-spam-bot-repeats').value, delay = +qs('#pttw-spam-bot-delay').value;
            abox.close();
            setTimeout(()=>{
                let i = 0;
    ...
    function addScriptUI(){
        let abox = box({
            ...
            text: `
                <div style="display: flex; justify-content: space-evenly;">
                    <button id="pttw-add-script-btn-file" class="btn btn-default">Загрузить из файла</button>
                    <button id="pttw-add-script-btn-url" class="btn btn-default">Загрузить по URL</button>
                </div><br>
                <textarea id="pttw-add-script-ta"
                style="font-family: Consolas, monospace; width: 20em; height: 5em; resize: none;"
                placeholder="JSON"></textarea>
                <div style="display: flex; justify-content: center;">
                    <button id="pttw-add-script-btn-ok" class="btn btn-success">Добавить</button>
                </div>
            `
        });
        let btnFile = qs('#pttw-add-script-btn-file'), btnURL = qs('#pttw-add-script-btn-url'), btnOk = qs('#pttw-add-script-btn-ok'), ta = qs('#pttw-add-script-ta');
        ...
        btnURL.addEventListener('click', ()=>{
            let bx = box({
                ...
                text: `
                    <input type="url" size="60" id="pttw-add-script-urld-inp" placeholder="URL"><br>
                    <div style="display: flex; justify-content: center;">
                        <button id="pttw-add-script-urld-btn-ok" class="btn btn-success">OK</button>
                    </div>
                `
            });
            let inp = qs('#pttw-add-script-urld-inp'), btn = qs('#pttw-add-script-urld-btn-ok');
            btn.addEventListener('click', ()=>{
                let val = inp.value + '';
                bx.close();
                (async ()=>{
    ...
    twOptions = localStorage.twOptions?JSON.parse(localStorage.twOptions):{};
    ...
    if(!localStorage.twScripts || q.tw_reset_scripts){
        localStorage.twScripts = '[]';
    }
    twScripts = JSON.parse(localStorage.twScripts);
    ...
    function startMenuLoaded(){
        ...
        if(localStorage.disableWsHook == 'false'){
            wshook(({socket,data})=>{
    ...
    function gameLoaded(){
        ...
        if(pt.chat.fta){
            ...
            pt.chat.fta = fta;
            ta.style.merge({ position: 'absolute', left: '-9999px', top: '-9999px' });
        ...
        if(!localStorage.trLang) localStorage.trLang = '0';
        ...
        pt.chat.registerCommand('tr', args=>{
            ...
            if(args.length > 1){
                ...
                localStorage.trLang = args[0];
            }
        });

        pt.chat.hook.attach('send', msg=>{
            if(localStorage.trLang == '0') return msg;

            let xhr = new XMLHttpRequest();
            xhr.open('GET', `https://nekit270.ch/pttw/api/translate.php?text=${encodeURIComponent(msg)}&from=auto&to=${localStorage.trLang}`, false);
            xhr.send();
        ...
        if(twOptions.color_picker){
            gl.canvas.addEventListener('click', e=>{
                if(e.altKey) alert(rgbToHex(pt.graphics.readPixel(e.x, e.y)));
            });
        ...
        setInterval(()=>{
            ...
            if(qs('.chat-log-tabs')){
                if(!qs('#pttw-chat-search-btn')){
                    let tabs = qs('.chat-log-tabs');
                    let el = ce('button');
                    el.style.merge({
                        marginLeft: '0.3em'
                    });
                    el.id = 'pttw-chat-search-btn';
